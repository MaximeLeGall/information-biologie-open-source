...
if($_GET['article']){
    try{
        ...
        $articles = $PDO -> query('SELECT id_article, fk_domain, article_content, DATE_FORMAT(article_registration, "%d/%m/%Y"), user_pseudo, article_name FROM article INNER JOIN t_users ON article.author_article = t_users.id_user WHERE id_article =' . $_GET['article']);
        
        $article = $articles->fetch(PDO::FETCH_ASSOC);
        $fk_article_domain = $article['fk_domain'];
